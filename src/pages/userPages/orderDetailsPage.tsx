...
console.log(id)
  ...
  const orderItem = orderData ? {
    ...
    status: orderData.orderStatus === 'pending' ? 'Pending' : orderData.orderStatus === 'completed' ? 'Shipped' : 'Delivered',
    seller: {
  ...
  />
          ...
          <div className="md:w-1/2 p-6">
            <h2 className="text-2xl font-semibold text-amber-900 mb-4">{orderItem.productName}</h2>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-2xl font-bold text-green-600">${orderItem.bidAmount.toFixed(2)}</span>
            </div>
            <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                orderItem.status === 'Pending' ? 'bg-yellow-200 text-yellow-800' :
                orderItem.status === 'Shipped' ? 'bg-blue-200 text-blue-800' :
        ...
        <div className="border-t border-amber-200 px-6 py-4">
          ...
          <div className="flex items-center">
            <img src={orderItem.seller.avatar} alt={orderItem.seller.name} className="w-12 h-12 rounded-full mr-4" />
            <div>
              <p className="font-medium text-amber-900">{orderItem.seller.name}</p>
              <div className="flex items-center mt-1">
                {renderStarRating(orderItem.seller.rating)}
                <span className="ml-2 text-sm text-amber-700">{orderItem.seller.rating.toFixed(1)}</span>
              </div>
            </div>
          </div>  
        </div>
        <div className="border-t border-amber-200 px-6 py-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-amber-800">{orderItem.shippingAddress.city}, {orderItem.shippingAddress.state} {orderItem.shippingAddress.zipCode}</p>
      ...
      {isImageEnlarged && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" onClick={() => setIsImageEnlarged(false)}>
          <div className="max-w-4xl max-h-full p-4">
            <img src={orderItem.productImage[0]} alt={orderItem.productName} className="max-w-full max-h-full object-contain" />
          </div>
        </div>
      )}
    </div>
